    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($category) {
                ...
                $edit = '<a title="' . __('Edit :x', ['x' => __('Category')]) . '" href="javascript:void(0) " class="action-icon edit-blog-category" data-bs-toggle="modal" data-bs-target="#edit-payment" id="' . $category->id . '" name="' . $category->name . '" status="' . $category->status . '"><i class="feather icon-edit-1 edit-blog-category"></i></a>&nbsp';
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Blog\Http\Controllers\BlogCategoryController@update')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('Modules\Blog\Http\Controllers\BlogCategoryController@delete') && $category->id != 1) {
                    $str .= $delete;
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn([
                ...
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Blog\Http\Controllers\BlogCategoryController@edit', 'Modules\Blog\Http\Controllers\BlogCategoryController@delete']),
