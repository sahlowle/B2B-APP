    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($coupons) {
                ...
                $edit = '<a title="' . __('Edit :x', ['x' => __('Coupon')]) . '" href="' . route('coupon.edit', ['id' => $coupons->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp';
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Coupon\Http\Controllers\CouponController@edit')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('Modules\Coupon\Http\Controllers\CouponController@destroy')) {
                    $str .= $delete;
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '10%',
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Coupon\Http\Controllers\CouponController@edit', 'Modules\Coupon\Http\Controllers\CouponController@destroy']),
