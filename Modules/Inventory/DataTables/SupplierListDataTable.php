    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($suppliers) {
                ...
                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\SupplierController@edit')) {
                    $str = '<a title="' . __('Edit') . '" href="' . route('supplier.edit', ['id' => $suppliers->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp;';
                }

                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\SupplierController@destroy')) {
