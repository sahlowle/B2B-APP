    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('reference', function ($transfer) {
                ...
                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\Vendor\TransferController@receive')) {
                    if ($transfer->status != 'Received') {
                        $str .= '<a title="' . __('Receive Inventory') . '" href="' . route('vendor.transfer.receive', ['id' => $transfer->id]) . '" class="action-icon"><i class="fas fa-cart-plus"></i></a>&nbsp;';
                    }
                }

                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\Vendor\TransferController@edit')) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('vendor.transfer.edit', ['id' => $transfer->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp;';
                }

                if (auth()->user()?->hasPermission('Modules\Inventory\Http\Controllers\Vendor\TransferController@destroy')) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '12%',
                ...
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Inventory\Http\Controllers\Vendor\TransferController@destroy', 'Modules\Inventory\Http\Controllers\Vendor\TransferController@receive', 'Modules\Inventory\Http\Controllers\Vendor\TransferController@edit']),
