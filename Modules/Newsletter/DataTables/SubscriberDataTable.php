    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('email', function ($subscriber) {
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Newsletter\Http\Controllers\SubscriberController@edit')) {
                    $str .= '<a title="' . __('Edit :x', ['x' => __('Subscriber')]) . '" href="' . route('subscriber.edit', ['id' => $subscriber->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp';
                }
                if (auth()->user()?->hasPermission('Modules\Newsletter\Http\Controllers\SubscriberController@destroy')) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '10%',
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Newsletter\Http\Controllers\SubscriberController@edit', 'Modules\Newsletter\Http\Controllers\SubscriberController@destroy']),
