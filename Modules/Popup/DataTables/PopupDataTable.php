    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($popup) {
                ...
                $edit = '<a title="' . __('Edit :?', ['?' => __('popup')]) . '" href="' . route('popup.edit', ['id' => $popup->id]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp';
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Popup\Http\Controllers\PopupController@show')) {
                    $str .= $show;
                }
                if (auth()->user()?->hasPermission('Modules\Popup\Http\Controllers\PopupController@edit')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('Modules\Popup\Http\Controllers\PopupController@destroy')) {
                    $str .= $delete;
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => __('Action'), 'width' => '12%',
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Popup\Http\Controllers\PopupController@edit', 'Modules\Popup\Http\Controllers\PopupController@destroy', 'Modules\Popup\Http\Controllers\PopupController@show']),
