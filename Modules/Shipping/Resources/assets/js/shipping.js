...
//add provider btn enable
$(document).on('click','#v-pills-provider-tab',function(){
    $("#add-provider-btn").removeClass('d-none');
    $("#filter-provider-btn").removeClass('d-none');
    $("#v-pills-help-tab").addClass('d-none');
});

$(document).on('click','#v-pills-zone-tab,#v-pills-class-tab,#v-pills-general-tab',function(){
    $("#add-provider-btn").addClass('d-none');
    $("#filter-provider-btn").addClass('d-none');
    $("#v-pills-help-tab").removeClass('d-none');
});
...
$(document).on('click', '.edit_provider', function () {
    ...
    var trackingBaseUrl = $(this).data('tracking_base_url');
    var trackingUrlMethod = $(this).data('tracking_url_method');
    var status = $(this).data('status');
    var logoUrl = $(this).data('logo_url');
    var fileId = $(this).data('file_id');
    $('#edit-provider').find('#provider_id').val(id);
    $('#edit-provider').find('#name').val(name);
    $('#edit-provider').find('#country_id').val(countryId).trigger('change');
    $('#edit-provider').find('#tracking_base_url').val(trackingBaseUrl);
    $('#edit-provider').find('#tracking_url_method').val(trackingUrlMethod).trigger('change');
    $('#edit-provider').find('#status').val(status).trigger('change');
    $('.provider-logo').attr('src', logoUrl);
    $('#file_id').val(fileId);

    removeModalScreenShakes($('#edit-provider'));
    // Show the modal
    $('#edit-provider').modal('show');
});

$(document).on('click', '.add_provider', function () {
    removeModalScreenShakes($('#add-provider'));
});

$(document).on('click','.delete_provider',function(){
    var action_url = $(this).data('action_url');
    removeModalScreenShakes($('#delete-provider'));
    $('#delete-provider').find('form').attr('action', action_url);
});

$(document).on("file-attached", ".has-thumbnail", function (e, data) {
    $("#provider-image").html(data.html)
});

function removeModalScreenShakes(modal){
    var style = modal.attr('style');
    if (style && style.includes('padding-right: 17px')) {
...
$(document).on('change', '#shipping_provider', function() {
    $('.shipping-provider-options').toggleClass('d-none', !$(this).is(':checked'));
})
