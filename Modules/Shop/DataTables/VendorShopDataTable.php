    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->editColumn('name', function ($shops) {
                ...
                $str = '';
                if (auth()->user()?->hasPermission('Modules\Shop\Http\Controllers\Vendor\ShopController@edit')) {
                    $str .= $edit;
                }
                if (auth()->user()?->hasPermission('Modules\Shop\Http\Controllers\Vendor\ShopController@destroy')) {
                    $str .= $delete;
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => __('Action'), 'width' => '5%',
                'visible' => auth()->user()?->hasAnyPermission(['Modules\Shop\Http\Controllers\Vendor\ShopController@edit', 'Modules\Shop\Http\Controllers\Vendor\ShopController@destroy']),
