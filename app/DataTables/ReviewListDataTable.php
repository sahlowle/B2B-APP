    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($reviews) {
                $str = '';
                if (auth()->user()?->hasPermission('App\Http\Controllers\ReviewController@view')) {
                ...
                if (auth()->user()?->hasPermission('App\Http\Controllers\ReviewController@edit')) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('review.edit', ['id' => $reviews->id]) . '" class="action-icon pr-5 edit_review" data-bs-toggle="modal" data-bs-target="#edit-review" data-id=' . $reviews->id . '><i class="feather icon-edit-1"></i></a>&nbsp;';
                }

                if (auth()->user()?->hasPermission('App\Http\Controllers\ReviewController@destroy')) {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => __('Action'), 'width' => '10%',
                'visible' => auth()->user()?->hasAnyPermission(['App\Http\Controllers\ReviewController@edit', 'App\Http\Controllers\ReviewController@destroy']),
