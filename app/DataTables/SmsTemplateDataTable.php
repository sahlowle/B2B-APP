    ...
    public function ajax(): JsonResponse
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($templates) {
                $str = '';
                if (auth()->user()?->hasPermission('App\Http\Controllers\SmsTemplateController@edit')) {
                    $str .= '<a title="' . __('Edit') . '" href="' . route('sms.template.edit', ['slug' => $templates->slug]) . '" class="action-icon"><i class="feather icon-edit-1"></i></a>&nbsp;';
                }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'action', 'name' => 'action', 'title' => '', 'width' => '5%',
                'visible' => auth()->user()?->hasAnyPermission(['App\Http\Controllers\SmsTemplateController@edit']),
