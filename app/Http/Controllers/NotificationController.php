...
class NotificationController extends Controller
{
    ...
    * User List
    ...
    public function setting()
    {
        ...
        $files = apply_filters('notification_files', $files);
        ...
        $classes = collect($files)
            ->map(function ($file) use ($namespace, $except) {
                ...
                // Try fallback from "Modules" if first attempt fails
                if (strpos($file, 'Modules') !== false) {
                    $relativePath = substr($file, strpos($file, 'Modules'));
                    $moduleClass = str_replace(['/', '.php'], ['\\', ''], $relativePath);
                    $classesToTry[] = $moduleClass;
                }

                foreach ($classesToTry as $tryClass) {
                    if (class_exists($tryClass) && ! in_array($tryClass, $except)) {
                        return $formatClass($tryClass);
                    }
                }
