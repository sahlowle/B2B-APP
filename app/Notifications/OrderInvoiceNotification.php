...
class OrderInvoiceNotification extends Notification
{
    ...
    public function replaceSmsVariables($body)
    {
        ...
        $customTax = $this->request->updatedCustomTaxFee($this->request, true);
        $customFee = $this->request->customFeeCalculations();
        $data = [
            ...
            '{grand_total}' => formatCurrencyAmount($this->request->total + $customTax + $customFee['feeTotal'] + $customFee['customTaxTotal']),
            ...
            '{tax_charge}' => formatCurrencyAmount($this->request->tax_charge + $customFee['customTaxTotal'] + $customTax),
