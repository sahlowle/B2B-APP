...
trait hasFiles
{
     ...
     * @return string
     */
    public function fileUrlQuery()
    {
        $file = \DB::table($this->objectType())
            ->join('object_files', 'object_files.object_id', '=', $this->objectType() . '.id')
            ->join('files', 'files.id', '=', 'object_files.file_id')
            ->where('object_files.object_type', $this->objectType())
            ->where($this->objectType() . '.id', '=', $this->id)
            ->select('files.file_name as url')
            ->first();
        ...
        if ($file && $file->url) {
            ...
            } elseif (! $isLocal && isFileExist($filePath)) {
                $image = \Storage::disk()->url($filePath);
            }
        }
