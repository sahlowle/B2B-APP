                ...
                <div class="row m-0 px-7 pb-30p">
                    <div class="col-md-3 p-0">
                        @if ($productAttr['attribute_id'])
                            ...
                            <input type="hidden" name="attribute_types[{{$loop->index}}]" value="{{ $productAttr['type'] ?? "dropdown" }}">
                        @else
                            <div class="form-group mb-2 mr-2">
                                <label class="control-label gray-89">{{ __('Name') }}:</label>
                                <input type="text" class="form-control inputFieldDesign attribute-name" name="attribute_names[{{ $loop->index }}]" value="{{ $productAttr['name'] }}">
                            </div>
                        @endif
                        <input type="hidden" name="attribute_position[{{ $loop->index }}]" value="{{ $loop->index }}">
                    ...
                    <div class="col-md-9 p-0-res attr">
                        ...
                        @if ($productAttr['attribute_id'] && count($product->attributeValues) > 0)
                            <input type="hidden" value="{{ $productAttr['attribute_id'] }}" class="attr_id" name="attribute_attr_id[{{ $loop->index }}]">
                            @php
                        ...
                        @else
                            <div class="col-md-6 p-0">
                                <div class="form-group mb-2 mr-2">
                                    <label class="control-label gray-89">{{__('Attribute Type')}}</label>
                                    <select name="attribute_types[{{ $loop->index }}]" class="form-control select2">
                                        <option value="dropdown" {{ isset($productAttr['type']) && $productAttr['type'] == 'dropdown' ? 'selected' : '' }}>{{ __("Dropdown") }}</option>
                                        <option value="color" {{ isset($productAttr['type']) && $productAttr['type'] == 'color' ? 'selected' : '' }}>{{ __("Color") }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="dropdown-custom-attributes-{{$loop->index}} mt-5">
                                <textarea name="attribute_values[{{ $loop->index }}]" class="form-control h-94p" rows="3"
                                    placeholder="{{ __('List of values separated by |') }}">{{ implode('|', $productAttr['value']) }}
                                </textarea>
                            </div>
                        @endif
                    </div>
