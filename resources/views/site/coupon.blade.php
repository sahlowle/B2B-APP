...
@section('content')
    ...
    <div class="layout-wrapper px-4 xl:px-0">
        <div class="md:flex gap-4 justify-between">
            <div class="3xl:w-1/2 md:w-1/3 w-full ltr:mr-4 ltr:lg:mr-0 rtl:ml-4 rtl:lg:ml-0">
    ...
    <div class="mt-60p margin-top-124 layout-wrapper px-4 xl:px-0">
    ...
    @if (count($exclusiveCoupons) > 0)
        <div class="margin-top-124 mt-28">
            ...
            <div class="md:-mt-0 bg-gray-26 lg:bg-none">
                <div id="card-div" class="flex flex-wrap md:mb-0 layout-wrapper px-4 xl:px-0 relative">
                    <div class="lg:-mt-364p -mt-20 mb-8 lg:mb-0 wow fadeInUp w-full grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6" data-wow-duration="1s"
                        data-wow-delay="400ms" data-wow-offset="0">
                        @foreach ($exclusiveCoupons as $topThreeCoupon)
                            <div class="lg:mb-2.5 mb-0">
                                <div class="border border-gray-2 cursor-pointer rounded-lg card-grow bg-white md:p-18p p-15p coupon-card">
                                    ...
                                    <ul
                                        ...
                                        @if (isset($topThreeCoupon->products) && count($topThreeCoupon->products) > 0)
                                            <li class="leading-6 more">
                                                {{ __('For these products') . ': ' . implode(', ', $topThreeCoupon->products->pluck('name')->all()) }}
                                            </li>
                                        @else
    ...
    @if (count($weeklyCoupons) > 0)
        <div>
            <div class="mb-5 section-header flex fade-in0 lg:mt-20 mt-30p layout-wrapper px-4 xl:px-0 wow fadeInUp"data-wow-duration="1s" data-wow-delay="400ms" data-wow-offset="0">
            ...
            <div class="fade-in0 layout-wrapper px-4 xl:px-0 wow fadeInUp grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6" data-wow-duration="1.5s"
                data-wow-delay="450ms" data-wow-offset="0">
                @foreach ($weeklyCoupons as $weeklyCoupon)
                    <div class="mb-2.5">
                        <div class="border border-gray-2 cursor-pointer rounded-lg card-grow bg-white md:p-18p md:pb-23p p-15p coupon-card">
                            ...
                            <ul class="mt-7 list-disc text-gray-10 md:text-sm text-13 roboto-medium font-medium ltr:pl-3.5 rtl:pr-3.5">
                                ...
                                @if (isset($weeklyCoupon->products) && count($weeklyCoupon->products) > 0)
                                    <li class="leading-6 more">
                                        {{ __('For these products') . ': ' . implode(', ', $weeklyCoupon->products->pluck('name')->all()) }}
                                    </li>
                                @else
                                    <li class="leading-6">{{ __('For all products') }}</li>
                                @endif
    ...
    @if (count($allCoupons) > 0)
        <div>
            <div class="lg:mb-5 section-header mb-2.5 flex fade-in0 lg:mt-60p mt-25p layout-wrapper px-4 xl:px-0 wow fadeInUp" data-wow-duration="1s"
                data-wow-delay="400ms" data-wow-offset="0">
                <div class="primary-bg-color w-2p mt-2p lg:h-23p h-21p ltr:mr-2 rtl:ml-2"></div>
                <p class="text-gray-12 uppercase dm-bold leading-22p font-bold lg:text-lg text-15">{{ __('All Coupons') }}
                </p>
            </div>
            <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 fade-in0 layout-wrapper px-4 xl:px-0 wow fadeInUp" data-wow-duration="1.5s"
                data-wow-delay="450ms" data-wow-offset="0">
                @foreach ($allCoupons as $coupon)
                    <div class="lg:mb-2.5 coupon-card">
                        <div class="border border-purple-1 purple-box cursor-pointer rounded-lg bg-purple-2 p-18p">
                            ...
                            <ul class="mt-7 list-disc text-gray-10 md:text-sm text-13 roboto-medium font-medium ltr:pl-3.5 rtl:pr-3.5">
                                ...
                                @if (isset($coupon->products) && count($coupon->products) > 0)
                                    <li class="leading-6 more">
                                        {{ __('For these products') . ' : ' . implode(', ', $coupon->products->pluck('name')->all()) }}
                                    </li>
                                @else
                                    <li class="leading-6">{{ __('For all products') }}</li>
                                @endif
