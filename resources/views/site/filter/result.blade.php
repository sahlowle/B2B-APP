...
<div class="flex search-result overflow-hidden">
    @php
        $isShowFilter = preference('filter_show_category', true) || preference('filter_show_price_range', true) || preference('filter_show_attribute', true) || preference('filter_show_brand', true) || preference('filter_show_rating', true);
    @endphp
    @if ($isShowFilter)
        ...
        <div class="md:w-1/4 w-64 md:mt-5 middle-sidebar-scroll md:relative z-20 md:z-0 md:h-auto h-full fixed top-0 md:top-auto md:right-auto right-0 overflow-y-scroll md:overflow-auto bg-white filter-mobile hidden md:block" id="filter_box_result">
            <div class="border rounded-md px-5 pt-30p pb-2.5">
                ...
                @endif
                {{--price range--}}
                @if (preference('filter_show_price_range', true))
                    <div class="mt-0.5">
                        {{--price range--}}
                        <div class="mt-0.5">
                            <div class="border-one pb-3">
                ...
                @endif
                {{--ratings--}}
                @if (preference('filter_show_rating', true))
                    <div>
                        <div class="border-one pb-3">
                            <h3 class="text-base roboto-medium font-medium text-gray-12 leading-5">
                                {{ __('Ratings') }}
    ...
    <div class="w-full md:w-76% mt-2.5 pb-14" id="res-loader-html">
        <div class="flex justify-end md:justify-between" id="search_top_nav">
            <span class="dm-sans text-lg hidden md:flex justify-items-center my-3 leading-6 text-gray-12 font-medium md:w-1/3 xl:w-1/2 {{ $isShowFilter ? 'ltr:md:pl-30p rtl:md:pr-30p' : '' }}" id="found_total_item">
            ...
            <div class="sm:col-span-5 md:col-span-5 lg:col-span-3 md:mt-0 filter-anim product-result"
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-x-4 lg:gap-x-30p gap-y-4 mt-1 relative {{ $isShowFilter ? 'ltr:lg:pl-30p ltr:md:pl-4 rtl:lg:pr-30p rtl:md:pr-4' : '' }}"
                ...
                @if ($res->records->pagination->total > $res->records->pagination->rows_per_page)
                    ...
                    @endphp
                    <div class="flex text-gray-700 justify-center pt-5 mt-25p border-t {{ $isShowFilter ? 'ltr:ml-30p rtl:mr-30p' : '' }}">
