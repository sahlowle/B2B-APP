        ...
        @php
            if (!empty($popup) && $routePage[$popup->page_link] == $routeName) {
                ...
                if ($content->background == 'Color') {
                    ...
                    $file = getBackgroundImage($popup->fileUrlQuery());
        ...
        <footer class="max-w-7xl mx-auto px-4 xl:px-0 py-14">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 {{ $direction[$footer['main']['direction']] }}">
                    ...
                    <div class="text-center sm:text-left order-{{ isset($footer['main']['about_us']['sort']) ? $footer['main']['about_us']['sort'] : 1 }}">
                        <div class="flex-shrink-0 md:mx-0 mx-auto">
                            @if ($footerLogo->objectFile)
                                <div class="flex items-center justify-center sm:justify-start mx-auto sm:mx-0">
                                    <a href="{{ route('site.index') }}" aria-label="{{ config('app.name') }} homepage">
                                        <img class="w-44" height="30.14" src="{{ $footerLogo->fileUrlQuery() }}" alt="{{ __('Image') }}">
                                    </a>
                                </div>
                            @endif

                            <p class="text-sm font-medium roboto-regular mt-5 leading-6 break-all">
                                {!! isset($footer['main']['about_us']['data']['short_details']) ? $footer['main']['about_us']['data']['short_details'] : '' !!}
                            </p>
                        </div>
                        @if ($paymentMethods->objectFile)
                            <div class="flex space-x-4 mt-6 items-center justify-center sm:justify-start">
                                <img class="w-full sm:w-auto" src="{{ $paymentMethods->fileUrlQuery() }}" alt="{{ __('Image') }}">
                            </div>
                        @endif
                                ...
                                @if ($googlePlay->objectFile)
                                    <div class="flex xs:justify-center md:justify-start">
                                        <a href="{{ isset($footer['main']['about_us']['data']['google_play_link']) ? $footer['main']['about_us']['data']['google_play_link'] : '' }}" class="mt-15p" target="_blank">
                                            <img class="w-263p sm:w-32" src="{{ $googlePlay->fileUrlQuery() }}" alt="{{ __('Image') }}">
                                        </a>
                                    </div>
                                @endif
                                @if ($appStore->objectFile)
                                    <div class="flex xs:justify-center md:justify-start">
                                        <a href="{{ isset($footer['main']['about_us']['data']['app_store_link']) ? $footer['main']['about_us']['data']['app_store_link'] : '' }}" class="md:mt-11p mt-15p" target="_blank">
                                            <img class="w-263p sm:w-32" src="{{ $appStore->fileUrlQuery() }}" alt="{{ __('Image') }}">
                                        </a>
                                    </div>
                                @endif
                            </div>
                ...
                @if (isset($footer['main']))
                    @foreach ($footer['main'] as $key => $value)
                        @if (in_array($key, ['useful_links', 'pages', 'categories']) && $value['status'])
                            <div class="order-{{ $value['sort'] }}">
                                <ul class="space-y-2 text-sm roboto-regular break-all">
                                    <p class="mb-4 font-semibold text-base dm-sans text-gray-900 break-all">
                                        {!! $value['title'] !!}
                                    </p>
                                    @foreach ($value['data'] ?? [] as $widget)
                                        <a href="{{ $widget['link'] }}">
                                            <li class="hover:text-orange-400 transition-colors break-all">
                                                {!! $widget['label'] !!}
                                            </li>
                                        </a>
                                    @endforeach
                ...
                @if (isset($footer['main']['contact_us']['status']) && $footer['main']['contact_us']['status'] == 1)
                    <div class="order-{{ isset($footer['main']['contact_us']['sort']) ? $footer['main']['contact_us']['sort'] : 1 }}">
                        @if (isset($footer['main']['contact_us']['title']) && !empty($footer['main']['contact_us']['title']))
                            <p class="mb-4 font-semibold text-base text-gray-900 break-all">
                        ...
                        <div class="space-y-4 text-sm">
                            @if (isset($footer['main']['contact_us']['data']['address']) && !empty($footer['main']['contact_us']['data']['address']))
                                <div class="flex items-center gap-3 break-all">
                                    ...
                                    <span class="roboto-regular">{!! $footer['main']['contact_us']['data']['address'] !!}</span>
                                </div>
                            @endif
                            ...
                            @if (isset($footer['main']['contact_us']['data']['email']) && !empty($footer['main']['contact_us']['data']['email']))
                                <div class="flex items-center gap-3 break-all">
                                    ...
                                    <a href="mailto:{!! $footer['main']['contact_us']['data']['email'] !!}" class="hover:text-orange-400 transition-colors roboto-regular">{!! $footer['main']['contact_us']['data']['email'] !!}</a>
                                </div>
                            @endif
                            ...
                            @if (isset($footer['main']['contact_us']['data']['phone']) && !empty($footer['main']['contact_us']['data']['phone']))
                                <div class="flex items-center gap-3 break-all">
                                    ...
                                    <a href="tel:{!! preg_replace('/\s+/', '', $footer['main']['contact_us']['data']['phone']) !!}" class="hover:text-orange-400 transition-colors roboto-regular">{!! $footer['main']['contact_us']['data']['phone'] !!}</a>
                                </div>
                            @endif
                        </div>

                        @if (isset($footer['main']['contact_us']['data']['social_title']) && !empty($footer['main']['contact_us']['data']['social_title']))
                            <p class="dm-regular text-base mt-6 break-all">{!! $footer['main']['contact_us']['data']['social_title'] !!}</p>
                        @endif
                            ...
                            <div class="flex items-center gap-3 flex-wrap">
                                @if (isset($footer['main']['contact_us']['data']['social_data']))
                                    @foreach ($footer['main']['contact_us']['data']['social_data'] as $data)
                                        @if (!empty($data['link']))
                                            <a href="{{ $data['link'] }}" target="_blank" rel="noopener" aria-label="{{ $data['label'] ?? 'Social' }}" class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-slate-200 hover:bg-slate-300 transition-colors">
...
<section style="color: {{ isset($footer['bottom']['text_color']) ? $footer['bottom']['text_color'] : '' }}; background: {{ isset($footer['bottom']['bg_color']) ? $footer['bottom']['bg_color'] : '' }}">
    @if (isset($footer['bottom']['status']) && $footer['bottom']['status'])
        <div style="border-top: 1px solid {{ isset($footer['bottom']['border_top']) ? $footer['bottom']['border_top'] : '' }}" class="border-gray-13">
            <div class="max-w-7xl mx-auto px-4 xl:px-0 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
                <p class="dm-sans">{!! isset($footer['bottom']['title']) ? $footer['bottom']['title'] : '' !!}</p>
                <a href="#top" class="inline-flex items-center gap-2 hover:text-orange-400 transition-colors">
                    <span>Back to top</span>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 19V5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M6 11l6-6 6 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </a>
            </div>
        </div>
    @endif
