    ...
    <div style="height: {{ $product['height'] }}px" class="flex justify-center items-center">
        @if ($product['badge'])
            <div class="absolute top-2.5 ltr:left-2.5 rtl:right-2.5">
                ...
                @endif
                @if($item->type == \App\Enums\ProductType::$Variable ? false : $item->isShowStockQuantity() && $outStock == false)
                    <p class="primary-bg-color z-30 h-4 relative text-gray-12 mb-2.5 px-2 flex items-center rounded-sm leading-3 roboto-medium font-medium text-8 whitespace-nowrap w-max">{{ __(':x left', ['x' => $item->getStockQuantity()]) }}</p>
                @endif
            </div>
        @endif
