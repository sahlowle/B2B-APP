...
<div class="md:mt-3 mt-2">
    <p class="dm-bold font-700">
        @if(!$product->isGroupedProduct())
            ...
            @endif
                <span class="text-2.5xl text-gray-12 {{ $product->isVariableProduct() ? "display-none" : '' }}" id="item_price">
                    {{-- {{ $offerFlag ? multiCurrencyFormatNumber($product->priceWithTax($displayPrice, 'sale', false)) : multiCurrencyFormatNumber($product->priceWithTax($displayPrice, 'regular', false)) }} --}}
                </span>
                <span class="text-28 text-gray-10 display-none">/</span>
            @if($offerFlag || $product->isVariableProduct())
                <span class="text-gray-10 line-through {{ $product->isVariableProduct() ? "display-none" : '' }}" id="item_offer_price">
                    {{-- {{ multiCurrencyFormatNumber($product->priceWithTax($displayPrice, 'regular', false)) }} --}}
                </span>
            @endif
