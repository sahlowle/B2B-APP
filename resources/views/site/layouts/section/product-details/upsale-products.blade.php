                            ...
                            <div style="height: {{ $productCard['height'] }}px" class="md:h-260p md:w-auto w-32 h-32 flex justify-center items-center">
                                    @if ($productCard['badge'])
                                        <div class="absolute top-2.5 ltr:left-2.5 rtl:right-2.5">
                                            @if ($product->isShowStockStatus() && $product->getStockStatus() == 'Out Of Stock')
                                                @php $outStock = true @endphp
                                                <p class="bg-pinks-2 relative w-max z-30 h-4 text-reds-3 mb-2.5 px-1.5 flex items-center rounded-sm leading-3 roboto-medium font-medium pt-2p text-8 whitespace-nowrap">{{ __('Out Of Stock') }}</p>
                                            @endif
                                            ...
                                            @endif
                                            @if($product->type == \App\Enums\ProductType::$Variable ? false : $product->isShowStockQuantity() && $outStock == false)
                                                <p class="primary-bg-color z-30 h-4 relative text-gray-12 mb-2.5 px-2 flex items-center rounded-sm leading-3 roboto-medium font-medium text-8 whitespace-nowrap w-max">{{ __(':x left', ['x' => $product->getStockQuantity()]) }}</p>
                                            @endif
                                        </div>
                                    @endif
