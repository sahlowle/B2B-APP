...
@section('content')
    @php
        // $address get from controller
        $countries = \Modules\GeoLocale\Entities\Country::select('id', 'name', 'code')->orderBy('name')->get();
        $country = \Modules\GeoLocale\Entities\Country::where('code', $address->country)->first();
        
        $states = empty($country) ? [] : $country->divisions()->get();

        if (! empty($country)) {
            $state = \Modules\GeoLocale\Entities\Division::where('country_id', $country->id)->where('code', $address->state)->first();
        }

        $cities = empty($state) ? [] : \Modules\GeoLocale\Entities\City::where('country_id', $country->id)->where('division_id', $state->id)->get();
    @endphp
    <main class="md:w-3/5 lg:w-3/4 w-full main-content flex flex-col flex-1 address-form" id="customer_address_edit">
        <p class="text-2xl text-black font-medium">{{ __("Edit Address") }}</p>
        <div class="md:w-3/5 lg:w-3/4 w-full mt-5">
            <form class="w-full" action="{{ route('site.address.update', ['id' => $address->id]) }}" method="post" id="editAddressForm" onsubmit="return formValidation()">
                @csrf
                        ...
                        @if (preference('address_first_name_visible', 1))
                            <div class="mb-5">
                                ...
                                <input class="border border-gray-300 rounded w-full h-11 font-medium text-sm text-gray-600 form-control focus:border-gray-300 px-2 mt-1.5"
                                    type="text" 
                                    name="first_name" 
                                    value="{{ !empty(old('first_name')) ? old('first_name') : $address->first_name }}"
                        ...
                        <div class="mt-6">
                            ...
                            <div class="flex mt-1.5 radio-buttons gap-3">
                                <label class="custom-radio">
                                    <input type="radio" class="hidden radio-test" name="type_of_place" value="home" @checked($address->type_of_place == 'home') />
                                ...
                                <label class="custom-radio">
                                    <input type="radio" class="hidden radio-test" name="type_of_place" value="office" @checked($address->type_of_place == 'office') />
                                    <span class="radio-btn lg:w-36 lg:h-14 border border-gray-300 rounded  cursor-pointer relative inline-block font-medium text-center lg:text-sm text-xs ltr:pl-5 rtl:pr-5 text-black bg-white">
                                        <svg class="ltr:right-0 rtl:left-0 bg-yellow-300 p-1 rounded-full absolute opacity-0 w-5 h-5 top-1"
                                            xmlns="http://www.w3.org/2000/svg" width="11" height="9"
                                            viewBox="0 0 11 9" fill="none">
