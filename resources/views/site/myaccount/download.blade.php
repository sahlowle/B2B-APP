...
@section('page_title', __('Downloads'))
@section('content')
    @php
    ...
    @endphp
    <main class="md:w-3/5 lg:w-3/4 w-full main-content flex flex-col flex-1" id="customer_download">
        <section>
            <div class="flex justify-between lg:items-center sm:gap-7 gap-3 flex-wrap">
                <p class="text-2xl text-black font-medium">{{ __('Downloads') }}</p>
            </div>
                ...
                <table class="w-full bg-white mt-3 customer-table">
                    <thead>
                        <tr class="border-gray-400 border rounded">
                            ...
                            <th class="py-2 font-normal text-black px-3 ltr:text-left rtl:text-right whitespace-nowrap text-sm">
                                {{ __('Expiry') }}</th>
                            <th class="py-2 font-normal text-black px-3 ltr:text-right rtl:text-left whitespace-nowrap text-sm">
                                {{ __('Action') }}
                            </th>
                        </tr>
                    </thead>
                            ...
                            @foreach ($orders as $order)
                                ...
                                @if(is_array($downloadData) && !empty($downloadData))
                                    @foreach ($order->download_data as $key => $data)
                                        @if($data['is_accessible'] == 1 && $order->checkValidFile($data))
                                            @php $isFound = true;  @endphp
                                            <tr>
                                                <td class="p-3 ltr:text-left rtl:text-right">
                                                    <span class="w-72 break-words h-11 flex items-center font-medium text-black text-sm">
                                                        {{ $data['name'] }}
                                                    </span>
                                                </td>
                                                <td class="p-3 ltr:text-left rtl:text-right">
                                                    <span class="font-medium text-sm text-black whitespace-nowrap">
                                                        {{ $data['download_limit'] == '' || $data['download_limit'] == -1 ? __('Unlimited') : $data['download_limit'] - $data['download_times'] }}
                                                    </span>
                                                </td>
                                                <td class="p-3 ltr:text-left rtl:text-right">
                                                    <span class="font-medium text-sm text-black whitespace-nowrap">
                                                        {{ $data['download_expiry'] == '' ? __('Never') : formatDate($order->created_at->addDays($data['download_expiry'])) }}
                                                    </span>
                                                </td>
                                                <td class="p-3 ltr:text-left rtl:text-right">
